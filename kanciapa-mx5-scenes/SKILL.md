...
- **Gate block inserted (2026-09-21)** — confirmed real on-device (2026-07-20 screenshots) as its own insertable model, not just a global param, and now actually placed as new block **#1**, ahead of Compressor (so compression doesn't amplify noise before the gate catches it, and gate doesn't chop delay/reverb tails later in the chain). **Noise Filter** (dual low/high-band, Low/High Thresh/Attack/Release, default −20dB/−20dB/0.99ms/100ms) is the documented fallback if Gate alone isn't precise enough — not needed so far.
- **Rig has an 11th block in use for Volume** (out of scope for the effects chain, kept as-is) — the "10 fixed blocks" table above predates the Gate insertion; effective current count is 11 effect blocks (Gate + the original 10) + Volume. Table not yet renumbered.
